...
try 
        ...
        for f=1:numel(files)
            ...
            %prediction = mhs_img_downscale2_mean(mhs_img_downscale2_mean(prediction));
            
            
            tic
            
            %prediction = mhs_img_downscale2_mean(mhs_img_downscale2_mean(mhs_img_downscale2_mean(prediction_mask,false),false),false);
            prediction = down_scale2(prediction_mask,shape2D);
            
            fprintf('fast downscaling took %d seconds\n',ceil(toc));
            
            shape2D_prg=size(prediction);
            
            % scale t = sigma^2 / smooth with e^{-x^2/(2t)}
            sigma=sqrt(shape2D_prg./shape2D)/sqrt(2);
            
            tic
            %tr_img(:,:,f)=imresize(datathon_im_smooth2D(single(prediction),sigma,'normalize',true),shape2D,'bilinear');
            tr_img(:,:,f)=imresize(pipeline_im_smooth2D(single(prediction),sigma,'normalize',false),shape2D,'bilinear');
            fprintf('Gaussian downscaling took %d seconds\n',ceil(toc));
            
            if weighted_img || weighted_img2
                tic
                prediction_tr=(imread([ifolder_tracer,'/',files(f).name]));
                %prediction_inj=(imread([ifolder_inj,'/',files(f).name]));
                
                %prediction_tr = max(prediction_tr,prediction_inj);
                ...
                if weighted_img
                    ...
                    tic
                    prediction = down_scale2(prediction,shape2D);
                    fprintf('fast downscaling took %d seconds\n',ceil(toc));
                    tic
                    tr_img_weighted(:,:,f)=imresize(pipeline_im_smooth2D(single(prediction),sigma,'normalize',false),shape2D,'bilinear');
                end
                if weighted_img2
                    ...
                    fprintf('fast downscaling took %d seconds\n',ceil(toc));
                    tic
                    tr_img_weighted2(:,:,f)=imresize(pipeline_im_smooth2D(single(prediction),sigma,'normalize',false),shape2D,'bilinear');
                end
        ...
        tr_img(tr_img(:)<0) = 0;
        %RGB_nifti  = imgBG;
        tr_img = smooth_tracer_3D(tr_img);
        ...
        if weighted_img
            ...
            %RGB_nifti  = imgBG;
            tr_img_weighted = smooth_tracer_3D(tr_img_weighted);
        ...
        if weighted_img2
            ...
            %RGB_nifti  = imgBG;
            tr_img_weighted2 = smooth_tracer_3D(tr_img_weighted2);
...
function img = down_scale2(img, target_shape)
    shape = size(img);
    if all ((shape/target_shape) >= 2 )
        img = mhs_img_downscale2_mean(img,false);
        img = down_scale2(img, target_shape);
    end
